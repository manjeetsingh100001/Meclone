...
// for in cart item modify
const CartModalItem = ({ item, handleDelete, handleSave  }) => {
    const [qty, setQty] = useState( 0 );
    const [price, setPrice] = useState( item.qty * item.d_price );
    ...
    useEffect(() => { 
        setPrice( qty * item.d_price );
    }, [item.d_price, qty]);

    return (
        <div className={styles.root}>
            <div className={itemStyles.root}>
                <img className={itemStyles.img} src={item.img} alt={item.title} />
                <div className={itemStyles.details}>
                    ...
                    <p className={itemStyles.price}>&#8377;{item.d_price}</p>
                </div>
